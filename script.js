...
$('.resetPoints').click(function () {
    ...
    $('.tieScore').text(noPoints);
    $('.blueScore').text(bluePoints);
});

$('.enSpiller').click(function () {
    if (clickedArr.length == 0) {
        $('.enSpiller').addClass('active');
        $('.toSpillere').removeClass('active');
        players = 1;
        $('.spillere').html('Spillere: <b>1</b>');
    } else {
        return;
    }
});

$('.toSpillere').click(function () {
    if (clickedArr.length == 0) {
        $('.enSpiller').removeClass('active');
        $('.toSpillere').addClass('active');
        players = 2;
        $('.spillere').html('Spillere: <b>2</b>');
    } else {
        return;
    }
});
...
function randomClick() {
    ...
    else if (!$('#fem').hasClass('x') && !$('#fem').hasClass('o') && Math.floor(Math.random() * 3) == 1) {
        $('#fem').click();
...
function trykke(dette) {
    ...
    if (redWon || blueWon || uavgjort) {
        console.log(`Rød: ${redPoints}\nBlå: ${bluePoints}\nUavgjort: ${noPoints}`);
        $('.redScore').text(redPoints);
        $('.tieScore').text(noPoints);
        $('.blueScore').text(bluePoints);
    }
...
function restart() {
    $('.restartButton').addClass('hideButton').removeClass('colRed').removeClass('colBlue').removeClass('colBlack').removeClass('showButton');
    $('.div').removeClass('blurPointerNone');
    $('.rute').removeClass('x').removeClass('o');
    $('.blueLine').remove();
    $('.redLine').remove();
    // person = "x";
    clickedArr = [];
